...
export async function POST(request: Request) {
  try {
    ...
    if (!models.length) {
      return NextResponse.json({ error: "API key không có model tạo nội dung phù hợp." }, { status: 422, headers: { "Cache-Control": "no-store" } });
    }
    return NextResponse.json({ valid: true, models }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
